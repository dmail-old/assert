...
var ValueAssertionHandler = proto.extend('ValueAssertionHandler', {
	...
	checkAssertionCompatibility(assertionName){
		var incompatibleAssertionName = this.incompatibilities.find(function(incompatibilityPair){
			var leftAssertion = incompatibilityPair[0];
			var rightAssertion = incompatibilityPair[1];

			if( assertionName === leftAssertion ) return rightAssertion;
			if( assertionName === rightAssertion ) return leftAssertion;
		});
