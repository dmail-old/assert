...
var moreAssertions = [
	function resolveWith(thenable, expectedResolutionValue){
		// du coup faut que j efasse une assertion au sein d'une assertion

		// fail if argument length is < 2 -> NOT_ENOUGH_ARGUMENTS
		// fail if argument length is > 2 -> TOO_MUCH_ARGUMENTS
		// fail if thenable is not a thenable -> INVALID_ARGUMENT
		// fail if thenable rejects, (it's even an error not only a failure) -> REJECTED
		// fail if thenable resolution value does not match expectedResolution value -> DIFFERENT | DIFFERENT_VALUES ....
		// resolveWith is an assertion, what I have to do it factorize the check of argument length, argument types
		// rejected, match etc

		/*

		this.assert('argumentLengthIs', 2);
		this.assert('isThenable', thenable);
		*/

		this.expectedResolutionValue = expectedResolutionValue;
